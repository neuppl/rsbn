...
pub unsafe extern "C" fn make_cpt(
    ...
    assigments: *const *const usize,
    probabilities: *const *const Probability,
) -> *mut CPT {
    ...
    let prob: HashMap<Assignment, Probability> = unsafe {
        ...
        let assign_vec: Vec<Vec<usize>> = assign
            .iter()
            .map(|x| std::slice::from_raw_parts(*x, num_parents + 1).to_vec())
            .collect();
